...
export default function GamificationCard() {
  ...
  const pct = (v: number) =>
    isNaN(v) ? 0 : Math.round((Math.max(0, Math.min(10, v)) / 10) * 100)
  ...
  return (
    <div
      className={`mx-auto max-w-md transition-transform duration-600 ease-out ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'}`}
    >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-gray-600">
              Resumo dos últimos {records7.length} registros
            </div>
          </div>
        ...
        <div className="mt-3">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              {submitMessage && (
                <div className="text-sm text-green-600">{submitMessage}</div>
              )}
            </div>
          </form>

          {loading && (
            <div className="mt-2 text-xs text-gray-500">Carregando...</div>
          )}
