...
export default function UploadProfileImage({ onUploadSuccess }: UploadProfileImageProps) {
  ...
  return (
    <section className="rounded-xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-4">
      <h3 className="mb-2 font-semibold text-[var(--text-primary)]">Foto de Perfil</h3>
      <input
        type="file"
        accept="image/*"
        onChange={upload}
      ...
      />
      {uploadError && <div className="mt-2 text-sm text-[var(--accent-danger)]">{uploadError}</div>}
    </section>
  )
}
