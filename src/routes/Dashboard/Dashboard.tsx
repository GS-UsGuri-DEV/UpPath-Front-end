  ...
  async function upload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (DB_ID && COLLECTION_USERS && userData?.$id) {
        ...
        try {
          const ownerEmail =
            (me as unknown as Record<string, unknown>)?.email ??
            displayEmail ??
            ''
          const ownerName =
          ...
          console.error(
            'Error creating user document and saving profile image',
            err,
          )
          setUploadError(String((err as Error)?.message ?? err))
        }
      ...
      if (
        msg.includes('Storage bucket with the requested ID could not be found')
      ) {
        setUploadError(
          'Erro: bucket do Storage não encontrado. Verifique `VITE_APPWRITE_BUCKET_PUBLIC` e se o bucket existe no painel do Appwrite.',
        )
      } else if (
        msg.includes('not authorized to perform the requested action')
      ) {
        setUploadError(
          'Erro: Sem permissão para upload. Configure as permissões do bucket no Appwrite Console.',
        )
      } else {
        setUploadError(msg)
      }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-6">
          ...
          {editMode ? (
            ...
            <>
              <div className="mt-3 space-y-1 text-sm">
                ...
                {String(
                  (userData as unknown as Record<string, unknown>)?.ocupacao ??
                    '',
                ) && (
                  <p>
                ...
                {String(
                  (userData as unknown as Record<string, unknown>)?.genero ??
                    '',
                ) && (
                  <p>
      ...
      <section className="rounded-xl border bg-white p-4">
        <h2 className="mb-2 font-semibold">Dados brutos (debug)</h2>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <h3 className="mb-1 text-xs font-medium">Account (me)</h3>
          ...
          <div>
            ...
            <div className="mt-2 text-xs">
              <p>
                fileUrl: <span className="break-all">{fileUrl ?? '—'}</span>
              </p>
              <p>
                profile_image (field):{' '}
              ...
              <p>
                lastFileId:{' '}
                <span className="break-all">{lastFileId ?? '—'}</span>
              </p>
              <p>
                BUCKET_PUBLIC usado:{' '}
                <span className="break-all">{BUCKET_PUBLIC}</span>
              </p>
              {uploadError && (
                <p className="text-sm text-red-600">Erro: {uploadError}</p>
              )}
              {uploadError &&
                (uploadError.includes('bucket do Storage não encontrado') ||
                  ...
                  <div className="mt-2 rounded border border-yellow-300 bg-yellow-50 p-2 text-xs">
                    ...
                    {uploadError.includes('Sem permissão para upload') ? (
                      <ol className="mt-1 ml-4 list-decimal">
                        <li>
                          Acesse Appwrite Console → Storage → Buckets →
                          private-media
                        </li>
                        <li>Vá em Settings → Permissions</li>
                        <li>
                          Em <strong>File create</strong>, adicione:
                          <br />- Role: <code>Users</code> (para usuários
                          logados)
                          <br />- Ou Role: <code>Any</code> (para qualquer um —
                          mais fácil para testes)
                        </li>
                        <li>
                          Em <strong>File read</strong>, adicione a mesma role
                          (para ver a imagem depois)
                        </li>
                      ...
                      <ol className="mt-1 ml-4 list-decimal">
                        ...
                        <li>
                          No projeto, crie/edite <code>.env.local</code> e
                          adicione:
                          <br />
                          <code>VITE_APPWRITE_BUCKET_PUBLIC=seu-bucket-id</code>
                        </li>
                        <li>
                          Reinicie o dev server: <code>pnpm run dev</code>
                        </li>
                      </ol>
                    )}
                  </div>
      ...
      <section className="rounded-xl border bg-white p-4">
        <h2 className="mb-2 font-semibold">Upload (Storage)</h2>
        <input type="file" onChange={upload} />
        {fileUrl && (
          <img src={fileUrl} alt="preview" className="mt-3 w-28 rounded" />
        )}
      </section>
    </div>
  )
